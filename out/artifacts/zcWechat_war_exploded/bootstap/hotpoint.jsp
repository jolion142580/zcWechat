         ...
         url:"QueueHist!"+id+"YearServerTakeRank.action?year="+year,  
          ...
          success:function(result){   //function1()
			...
			for(var i=0;i<rs.length;i++){
					...
					var count = rs[i].count;
					var sc = rs[i].SSerial;
					var str = '{"value":"'+count+'","label": "'+servername+'","link":"javascript:myJS('+sc+')"}';
					if(i==rs.length-1){
						datastr = datastr + str ;
					}else{
						datastr = datastr + str +",";
...
function ajaxXmlYear(id,year,name){
    ...
    $.ajax({   
          type:"POST",  
          url:"QueueHist!getonlyServerYear.action?year="+year+"&address="+id+"&name="+name,  
           dataType:"text",  
          success:function(result){   //function1()
			///alert(datastr);
			getChart3(result);
