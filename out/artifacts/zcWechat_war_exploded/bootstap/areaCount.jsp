...
function getChart1(dataString) {
		...
		var chart = new FusionCharts("easyui/FusionCharts_XT_Evaluation/Charts/MSLine.swf","chartId",width, height, "0", "0");
...
function getChart2(dataString) {
		...
		var chart = new FusionCharts("easyui/FusionCharts_XT_Evaluation/Charts/Line.swf","chartId3_flash",
				width, height, "0", "0");
...
function ajaxXml(id,year,month){
  ...
  $.ajax({   
          type:"POST",  
         url:"QueueHist!getArearCountJson.action?year="+year+"&address="+id+"&month="+month,  
           dataType:"text",  
...
function ajaxServerXml(value,value1){
	//获得狮山情况
  $.ajax({   
          type:"POST",  
         url:"QueueHist!getDayTime.action?year="+value+"&address="+value1,  
           dataType:"text",  
          success:function(result){   //function1()
			///alert(datastr);
			getChart2(result);
