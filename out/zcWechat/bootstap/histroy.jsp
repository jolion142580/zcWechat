        ...
        <div id="mws-container" class="clearfix">
            ...
            <div class="container" >
            	...
            	<div class="mws-panel grid_4">
                	<div class="mws-panel-header">
                    	<span class="mws-i-24 i-graph">业务量环比（点击柱状图查看详情）</span>
                    </div>
                    <div class="mws-panel-body">
            	...
            	<div class="mws-panel grid_4">
                	<div class="mws-panel-header">
                    	<span class="mws-i-24 i-books-2">业务量同比（点击曲线标点查看详情）</span>
                    </div>
                    <div class="mws-panel-body" style="width:100%; height:435px;font-size:10px">
                        <div class="mws-panel-content" style="padding:0px">
	                    	<div id="chart2" style="width:100%; height:435px;"></div>
                        </div>
                    </div>
                </div>
...
//月业务量函数
function myJS(value,mytitle,width){
	var year = $(".selector").val();
	var id = $(".current").attr("id");
	ajaxXmlMonth(id,year,value);
	///
...
//年业务量函数
function yearJS(value){
	var id = $(".current").attr("id");
	ajaxXmlYear(id,value);
	///
	$("#mws-jui-dialog").dialog({
		autoOpen: false, 
		title: "年业务量", 
...
//图形2
function getChart2(dataString) {
		if  ( FusionCharts( "chartId1" ) )  FusionCharts( "chartId1" ).dispose();
		var jsondata ='{"chart": {"showpercentvalues": "0","showlegend": "0","showlabels": "1","showvalues": "1","showpercentageinlabel": "0","formatNumberScale": "0","unescapeLinks":"0"},"data": '+dataString+'}';
		var d = document.getElementById("chart2");
		var width = d.offsetWidth ;
		var height = d.offsetHeight ;
		var chart = new FusionCharts("easyui/FusionCharts_XT_Evaluation/Charts/Line.swf","chartId1",
				width, height, "0", "0");
		chart.setJSONData( jsondata );
...
function getChart3(dataString) {
		...
		var chart = new FusionCharts("easyui/FusionCharts_XT_Evaluation/Charts/Pie2D.swf","chartId3_flash",
				width, height, "0", "0");
		chart.setJSONData( jsondata );
...
function getChart4(dataString) {
		if  ( FusionCharts( "chartId4" ) )  FusionCharts( "chartId4" ).dispose();
		var jsondata ='{"chart": {"bgcolor": "406181, 6DA5DB","bgalpha": "100","basefontcolor": "FFFFFF","canvasbgalpha": "0","canvasbordercolor": "FFFFFF","divlinecolor": "FFFFFF","divlinealpha": "100","numvdivlines": "10","vdivlineisdashed": "1","showalternatevgridcolor": "1","linecolor": "BBDA00","anchorradius": "4","anchorbgcolor": "BBDA00","anchorbordercolor": "FFFFFF","anchorborderthickness": "2","showvalues": "0","showvalues":"1","tooltipbgcolor": "406181","tooltipbordercolor": "406181","alternatehgridalpha": "5","unescapeLinks":"0"},"data": '+dataString+'}';
		var d = document.getElementById("chart_more");
		var width = d.offsetWidth ;
		var height = d.offsetHeight ;
		var chart = new FusionCharts("easyui/FusionCharts_XT_Evaluation/Charts/Column2D.swf","chartId4",width, height, "0", "0");
		chart.setJSONData( jsondata );
		///chart.setDataURL(url);
		 chart.render("chart_more");
	}
...
function ajaxXml(id,year){
  ...
  $.ajax({   
          type:"POST",  
         url:"QueueHist!getMonthCountByYearJson.action?year="+year+"&address="+id,  
           dataType:"text",  
    ...
    $.ajax({   
          type:"POST",  
         url:"QueueHist!getYearCountByYearJson.action?year="+year+"&address="+id,  
           dataType:"text",  
          success:function(result){   //function1()
...
//月业务量
function ajaxXmlMonth(id,year,month){
	//获得狮山情况

    $.ajax({   
          type:"POST",  
          url:"QueueHist!getServerTypeMonthCountSByTodayJson.action?year="+year+"&address="+id+"&month="+month,  
           dataType:"text",  
          success:function(result){   //function1()
			///alert(datastr);
			getChart3(result);
...
//年业务量
function ajaxXmlYear(id,year){
	//获得狮山情况
  $.ajax({   
          type:"POST",  
         url:"QueueHist!"+id+"YearServerTakeRank.action?year="+year,  
           dataType:"text",  
          success:function(result){   //function1()
		          	eval("var rs ="+result);
			var datastr ='[';
			for(var i=0;i<rs.length;i++){
					...
					///var count = rs[i].count;
					var servername = rs[i].servername.replaceAll("\\\"","").replaceAll("'","").replaceAll(",","");
					if(servername.length>8){
						servername = servername.substring(0,8)+"..."
					}
					var count = rs[i].count;
					var str = '{"value":"'+count+'","label": "'+servername+'"}';
					if(i==rs.length-1){
						datastr = datastr + str+"]" ;
					}else{
						datastr = datastr + str +",";
